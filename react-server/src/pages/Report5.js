...
function Report5Content() {
  ...
  //Select Montage
  function fetchHypnogramData() {
    fetch("/hypnogramChart/P0003_24062022_025830.edf/0/3")
      .then((res) => res.json())
      .then((data) => {
  ...
  async function fetchChannelData(chartLabel) {
    let chart = [];
    await fetch(
      "/channelCharts/P0003_24062022_025830.edf/" + chartLabel + "/0/15360"
    )
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
        ...
        >
          ...
          >
            ...
            {/* {charts == undefined
              ? null
              : charts.map((row) => (
                  <CanvasJS chartLabel={row.chartLabel} chartData={row.data} />
                ))} */}
          </Paper>
          {/* Sleep Stage */}
          {/* <Paper
